...
class RbacMiddleware(MiddlewareMixin):
    ...
    def process_request(self,request):
        ...
        permission_url_dict = request.session.get(settings.PERMISSIONS_URL_DICT_KEY)

        if not permission_url_dict:
            return redirect('/login/')

        # flag = False
        # for groupid,code_url in permission_url_dict.items():
